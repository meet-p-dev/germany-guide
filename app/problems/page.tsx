  ...
  return (
    <div className="space-y-6">
      <div className="space-y-2">
        <h1 className="text-3xl font-bold">Problems &amp; solutions</h1>
      ...
      <div className="flex flex-wrap gap-2 text-sm">
        <Link
          href="/problems"
          className={`rounded-full border px-3 py-1 ${!category ? "bg-primary text-primary-foreground" : "hover:bg-accent"}`}
        >
          All
        </Link>
        {categories.map((c) => (
          <Link
            ...
            className={`rounded-full border px-3 py-1 ${category === c.slug ? "bg-primary text-primary-foreground" : "hover:bg-accent"}`}
          >
            {c.name_en}
          </Link>
        ))}
            ...
            <li key={p.id}>
              ...
              >
                <div className="mb-1 flex flex-wrap items-center gap-2">
                  ...
                  {p.severity && (
                    <span
                      className={`rounded-full px-2 py-0.5 text-xs ${SEVERITY_STYLE[p.severity] ?? ""}`}
                    >
                      {p.severity}
                    </span>
                  )}
